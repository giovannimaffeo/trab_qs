...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-myapp3app-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-myapp3app-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-myapp3app-error'], {
        param: decodeURIComponent(error.response.headers['x-myapp3app-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-myapp3app-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-myapp3app-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-myapp3app-alert'], {
      param: decodeURIComponent(response.headers['x-myapp3app-params'].replace(/\+/g, ' ')),
    });
  }
}
