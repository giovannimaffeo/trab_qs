<template>
  <div>
    <h2 class="jh-entity-heading" data-cy="choosePizzaProcessDetailsHeading">
      <span v-text="$t('myappApp.choosePizzaProcess.home.title')">ChoosePizzaProcess</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('myappApp.choosePizzaProcess.home.refreshListLabel')">Refresh List</span>
        </button>

        <router-link :to="`/process-definition/PizzaFlowProcess/init`" tag="button" class="btn btn-primary mr-2">
          <font-awesome-icon icon="plus"></font-awesome-icon>
          <span v-text="$t('myappApp.choosePizzaProcess.home.createLabel')"> Create a new Travel Plan Process Instance </span>
        </router-link>
      </div>
    </h2>
    <br />
    <div class="alert alert-warning" v-if="!isFetching && choosePizzaProcessList && choosePizzaProcessList.length === 0">
      <span v-text="$t('myappApp.choosePizzaProcess.home.notFound')">No choosePizzaProcess found</span>
    </div>
